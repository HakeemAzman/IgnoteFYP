...
public class APOLLOReflectionDrop : MonoBehaviour {
    ...
    public bool ViewReflectionDropPosition = true;


    [Header ("Cubemap Offset")]
    ...
    public float NearPlaneDistance = 0.4f;
	...
	public void UpdateCubemap (int faceMask) {
		...
		if(( !cam )  || (cam.name == "CubemapCamera")) {
			GameObject go = new GameObject ("CubemapCamera",  typeof(Camera) );
			go.hideFlags = HideFlags.HideAndDontSave;
			go.transform.position = transform.position;
			go.transform.rotation = Quaternion.identity;
			cam = go.GetComponent<Camera>();
			cam.farClipPlane = FarPlaneDistance; // Set the rendering distance
            cam.nearClipPlane = NearPlaneDistance;
		...
		cam.transform.position = transform.position+PossitionOffset;
	...
	public void OnDrawGizmosSelected () {
        ...
        if (ViewClipPlaneDistance == true)
        {
            ...
            Gizmos.DrawWireCube(transform.position + PossitionOffset, new Vector3(FarPlaneDistance, FarPlaneDistance, FarPlaneDistance));

            Gizmos.color = Color.yellow;
            Gizmos.DrawWireCube(transform.position + PossitionOffset, new Vector3(NearPlaneDistance, NearPlaneDistance, NearPlaneDistance));
        }

            if (ViewReflectionDropPosition == true)
            {
                Gizmos.color = Color.blue;
                Gizmos.DrawSphere(transform.position + PossitionOffset, 0.1f);
            }
